...
Editor::Editor(Vector2 position, Vector2 size, std::vector<std::string>* buffer): Component(position, size), buf(buffer) {
	keypad(win, TRUE);
}

void Editor::refresh(bool force) {
	bool redrawY = true;
	bool redrawX = true;
	...
	if (redrawY || redrawX || force) {
		offset = tabOffset((*buf)[cursor.y], cursor.x);
		werase(win);
	...
	wmove(win, cursor.y - start.y, cursor.x + offset - start.x);
}
